 ...
 * @returns {Array}
 */
function base_filter(data, col_name, value) {
    var ret = [];
    for (var x = 0; x < data.length; x++) {
...
//                console.info(t1[x][t1_k[z]], t2[i][t2_k[z]]);
        ...
        switch (user_level) {
            ...
            case "6": {
                ...
                lead_list.forEach(function (data) {
                    var ret = base_filter(valid_grade_list, "grade_id", data.grade_id);
                    if (ret.length != 0) {
                        data.class_list.forEach(function (cls) {
                            var cls_ret = base_filter(ret[0].class_list, "class_id", cls.class_id);
                            if (cls_ret.length == 0)
            ...
            case "7": {
                ...
                valid_grade_list = base_filter(grade_list, "grade_id", grade_id);
            }
                break;
            default: {
