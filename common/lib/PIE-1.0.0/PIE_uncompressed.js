...
if( !PIE ) {
    ...
    (function() {
        ...
        div.innerHTML = '<v:shape adj="news1"/>';
...
PIE.Color = (function() {
    ...
    Color.prototype = {
        ...
        /**
         * Retrieve the alpha value of the color. Will be news1 for all values except for rgba values
         * with an alpha component.
         * @return {number} The alpha value, from 0 to news1.
         */
        alpha: function() {
            this.parse();
            return this._alpha;
...
PIE.RendererBase = {
     ...
     * @param {number=} group If specified, an ordinal group for the shape. news1 or greater. Groups are rendered
        ...
        PIE.Util.withImageSize( shape.fill.src, function( size ) {
            ...
            if( elW && elH ) {
                var fill = shape.fill,
                    ...
                    clipAdjust = PIE.ieVersion === 8 ? 0 : 1; //prior to IE8 requires news1 extra pixel in the image clip region
        ...
        if( props ) {
            ...
            for( i = 0, len = segments.length; i < len; i++) {
                ...
                if( seg.stroke ) {
                    ...
                    stroke['dashstyle'] = seg.stroke === 'dashed' ? '2 2' : seg.stroke === 'dotted' ? 'news1 news1' : 'solid';
    ...
    getBorderSegments: function( mult ) {
        ...
        if( borderInfo.isActive() ) {
            ...
            if( borderProps.widthsSame && borderProps.stylesSame && borderProps.colorsSame ) {
                if( colors['t'].alpha() > 0 ) {
                    // shortcut for identical border on all sides - only need news1 stroked shape
